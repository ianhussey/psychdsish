...
- **`dataset_description.json` (more psych-DS compliant).** Machine-readable metadata in the project root, required by the [psych-DS](https://psych-ds.github.io/) standard. `code/study_1/processing.qmd` contains a chunk (not run by default) that writes it from the codebooks with `psychdsish::write_dataset_description()`, so each variable is still described only once. Data file names in this project already follow the psych-DS convention of `key-value` pairs ending in `_data`. To check the project against the standard itself, use the psych-DS validator at <https://psych-ds.github.io/validator/>.
