...
A codebook (data dictionary) describes every variable in the processed data. The chunk below creates `../data/processed/stage-processed_codebook.csv` from `data_processed`, or updates it if it already exists. The automatic columns (`type`, `n_missing`, `values`) are refreshed on every render. The manual columns (`description`, `units`, `coding`) start as "TO BE COMPLETED MANUALLY": open the .csv (e.g., in Excel), replace them for every variable (write "none" where a column does not apply), and save it as .csv. Your entries are kept when the codebook is updated.
...
# write.csv(data_processed, "../data/processed/stage-processed_data.csv", row.names = FALSE)

placeholder <- "TO BE COMPLETED MANUALLY"
codebook_path <- "../data/processed/stage-processed_codebook.csv"
