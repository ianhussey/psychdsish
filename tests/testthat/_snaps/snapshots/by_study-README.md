...
## Structure
```
study_1/              # one folder per study, each with the same structure:
...
### Workflow
Each study has its own folders and follows the same workflow. Paths below are for study 1; the other studies' folders are named `study_2/`, `study_3/`, etc.
- Raw data lives in `study_1/data/raw/` and is never modified by code.
- `study_1/code/processing.qmd` reads the raw data and writes cleaned datasets to `study_1/data/processed/`. Rendering it also creates `study_1/code/processing.html`.
- `study_1/code/analysis.qmd` reads the processed data and writes plots to `study_1/data/outputs/plots/`, fitted model objects to `study_1/data/outputs/fitted_models/`, and tables to `study_1/data/outputs/results/`. Rendering it also creates `study_1/code/analysis.html`.
...
Rendering stops at the first error, so analyses never run on stale or partially processed data. Each file runs with its own folder as the working directory, so paths in the code are relative to the file's folder (e.g., from `study_1/code/`, raw data is in `../data/raw/`).
